...
const SUPABASE_ANON_KEY = window.G3D_PUBLIC_KEY || "";
...
const state = {
  session: null,
  page: "dashboard",
  cache: { clientes: [], estoque: [], orcamentos: [], pedidos: [], producoes: [], parametros: null, loja: null }
};

const navPages = [["dashboard", "Dashboard"], ["loja", "Minha loja"], ["parametros", "Parâmetros"], ["clientes", "Clientes"], ["estoque", "Estoque"], ["orcamentos", "Orçamentos"], ["pedidos", "Pedidos"], ["producao", "Produção"]];
...
function authMessage(error) {
  ...
  if (text.includes("invalid api key")) return "A conexão foi atualizada. Recarregue a página com Ctrl + F5 e tente novamente.";
...
function renderAuth(mode = "login") {
  ...
  document.getElementById("authForm").addEventListener("submit", async event => {
    ...
    const result = mode === "signup" ? await supabaseClient.auth.signUp({ email, password }) : await supabaseClient.auth.signInWithPassword({ email, password });
...
async function startApp() { renderApp(); await loadAll(); }
...
async function loadAll() {
  await Promise.all([loadTable("clientes"), loadTable("estoque"), loadTable("orcamentos"), loadTable("pedidos"), loadTable("producoes"), loadSingle("parametros_precificacao", "parametros"), loadSingle("loja_perfis", "loja")]);
  renderPage();
}
async function loadTable(table) { const { data, error } = await supabaseClient.from(table).select("*").is("deleted_at", null).order("created_at", { ascending: false }); if (!error) state.cache[table] = data || []; }
async function loadSingle(table, key) { const { data, error } = await supabaseClient.from(table).select("*").maybeSingle(); if (!error) state.cache[key] = data || null; }
...
function renderDashboard(el) {
  ...
  el.innerHTML = `<div class="page-head"><div><h1>Dashboard</h1><p class="muted">Visão geral do negócio</p></div></div><div class="grid stats"><div class="stat"><span>Clientes</span><strong>${state.cache.clientes.length}</strong></div><div class="stat"><span>Orçamentos</span><strong>${state.cache.orcamentos.length}</strong></div><div class="stat"><span>Pedidos</span><strong>${state.cache.pedidos.length}</strong></div><div class="stat"><span>Receita registrada</span><strong>${money(receita)}</strong></div></div><div class="grid" style="grid-template-columns: repeat(auto-fit, minmax(260px, 1fr)); margin-top: 14px;"><div class="card"><h3>Central de notícias</h3><p class="muted">Área reservada para comunicados, atualizações e parceiros do G3D Pro.</p></div><div class="card"><h3>Estoque crítico</h3><p class="muted">${critico.length ? critico.map(item => escapeHtml(item.nome)).join(", ") : "Nenhum item crítico."}</p></div></div>`;
}
...
function renderParametros(el) {
  renderSimpleForm(el, "Parâmetros", "Base de cálculo para orçamentos.", ["custo_kwh:Custo kWh", "consumo_kw_hora:Consumo kW/h", "custo_hora_maquina:Custo hora máquina", "custo_grama_padrao:Custo padrão por grama", "margem_percentual:Margem %", "taxa_minima:Taxa mínima", "pos_processamento_hora:Hora pós-processamento", "embalagem_padrao:Embalagem padrão"], state.cache.parametros || defaultParams(), async payload => {
...
function renderSimpleForm(el, title, subtitle, fields, row, onSave, type = "text") {
  el.innerHTML = `<div class="page-head"><div><h1>${title}</h1><p class="muted">${subtitle}</p></div></div><form class="card" id="simpleForm"><div class="form-grid">${fields.map(item => { const [key, label] = item.split(":"); const isLong = ["endereco", "observacao_padrao"].includes(key); return `<div class="field ${isLong ? "span-2" : ""}"><label>${label}</label>${isLong ? `<textarea name="${key}">${escapeHtml(row[key] || "")}</textarea>` : `<input type="${type}" step="0.01" name="${key}" value="${escapeHtml(row[key] ?? "")}" />`}</div>`; }).join("")}</div><div class="modal-foot"><button class="btn primary" type="submit">Salvar</button></div></form>`;
  document.getElementById("simpleForm").addEventListener("submit", event => { event.preventDefault(); onSave(Object.fromEntries(new FormData(event.currentTarget).entries())); });
}
...
function openForm(module, row = {}) {
  ...
  document.getElementById("recordForm").addEventListener("submit", async event => { event.preventDefault(); const payload = Object.fromEntries(new FormData(event.currentTarget).entries()); module.fields.forEach(spec => { const [key,, kind] = spec.split(":"); if (kind === "number") payload[key] = Number(payload[key] || 0); }); await saveRecord(module.table, payload, row.id); close(); });
}

function fieldHtml(row) {
  return spec => { const [key, label, kind] = spec.split(":"); const type = ["number", "date", "email"].includes(kind) ? kind : "text"; const required = kind === "required" ? "required" : ""; const value = escapeHtml(row[key] ?? ""); if (kind === "textarea") return `<div class="field span-2"><label>${label}</label><textarea name="${key}">${value}</textarea></div>`; return `<div class="field"><label>${label}</label><input type="${type}" step="0.01" name="${key}" value="${value}" ${required} /></div>`; };
}
...
function openBudgetForm(row = {}) {
  ...
  const backdrop = document.createElement("div");
  backdrop.className = "modal-backdrop";
  backdrop.innerHTML = `<form class="modal" id="budgetForm"><div class="modal-head"><strong>${row.id ? "Editar" : "Novo"} orçamento</strong><button class="btn" type="button" id="closeBudget">Fechar</button></div><div class="modal-body"><div class="form-grid"><div class="field"><label>Número</label><input name="numero" value="${escapeHtml(row.numero || nextNumber("ORC", state.cache.orcamentos.length))}" /></div><div class="field"><label>Status</label><input name="status" value="${escapeHtml(row.status || "rascunho")}" /></div><div class="field span-2"><label>Projeto</label><input name="projeto" value="${escapeHtml(row.projeto || "")}" required /></div><div class="field"><label>Material</label><input name="material" value="${escapeHtml(row.material || "PLA")}" /></div><div class="field"><label>Cor</label><input name="cor" value="${escapeHtml(row.cor || "Preto")}" /></div><div class="field"><label>Peso g/ml</label><input type="number" step="0.01" name="peso_g" value="${escapeHtml(row.peso_g || 0)}" /></div><div class="field"><label>Tempo h</label><input type="number" step="0.01" name="horas" value="${escapeHtml(row.horas || 0)}" /></div><div class="field"><label>Pós-processamento h</label><input type="number" step="0.01" name="pos_horas" value="${escapeHtml(row.pos_horas || 0)}" /></div><div class="field"><label>Total</label><input type="number" step="0.01" name="total" value="${escapeHtml(value)}" /></div><div class="calc-box span-2" id="calcBox">Preencha peso e tempo para calcular automaticamente.</div><div class="field span-2"><label>Observações</label><textarea name="observacao">${escapeHtml(row.observacao || "")}</textarea></div></div></div><div class="modal-foot"><button class="btn" type="button" id="cancelBudget">Cancelar</button><button class="btn primary" type="submit">Salvar orçamento</button></div></form>`;
  document.body.appendChild(backdrop);
