(function () {
  ...
  try {
    ...
    generateOrderFromBudget = async function generateOrderFromBudgetCoreFinal(row) {
      if (!row?.id) return;
    ...
    generateProductionFromOrder = async function generateProductionFromOrderCoreFinal(order) {
      if (!order?.id) return;
    ...
    const previousPdf = typeof openBudgetPdf === "function" ? openBudgetPdf : null;
    openBudgetPdf = async function openBudgetPdfCoreFinal(row) {
      if (!row) return;
      try {
        ...
        if (previousPdf) return previousPdf(row);
        showToast(`Não foi possível abrir o PDF: ${error.message || error}`);
      }
    };

    window.G3D_CORE_FINAL = true;
  } catch (error) {
    window.G3D_CORE_FINAL = false;
    console.error("G3D core final", error);
  }
})();
